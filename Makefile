...
pkgdatadir = $(datadir)/cmatrix
pkglibdir = $(libdir)/cmatrix
pkgincludedir = $(includedir)/cmatrix
...
EXTRA_DIST = AUTHORS COPYING ChangeLog INSTALL NEWS README 		TODO install-sh missing mkinstalldirs matrix.fnt 		matrix.psf.gz mtx.pcf cmatrix.1 cmatrix.spec
...
cmatrix_OBJECTS =  cmatrix.o
cmatrix_LDADD = $(LDADD)
cmatrix_DEPENDENCIES = 
cmatrix_LDFLAGS = 
debugCFLAGS = -ggdb3 -Wall -pg -O0
CFLAGS = -O2 $(debugCFLAGS)
...
Makefile.am Makefile.in NEWS TODO acconfig.h aclocal.m4 cmatrix.spec.in \
...
cmatrix.spec: $(top_builddir)/config.status cmatrix.spec.in
...
cmatrix: $(cmatrix_OBJECTS) $(cmatrix_DEPENDENCIES)
	@rm -f cmatrix
	$(LINK) $(cmatrix_LDFLAGS) $(cmatrix_OBJECTS) $(cmatrix_LDADD) $(LIBS)
